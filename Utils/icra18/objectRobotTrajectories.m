...
nSteps = 10;
for i=2:nSteps
    object1Pose(:,i) = RelativeToAbsolutePoseR3xso3GlobalFrame(object1Pose(:,i-1),...
        object1RelativePose);
end
...
rotationMatrix = rot([0;0;-pi/60]);
translationVector = [-0.25;0.15;0];
...
nSteps = 10;
for i=2:nSteps
    object2Pose(:,i) = RelativeToAbsolutePoseR3xso3GlobalFrame(object2Pose(:,i-1),...
        objectRelativePose);
end
%SE3 motion in object frame
% 0^H^{-1} = 0^L_{k+1} k^H^{-1} 0^L^{-1}_{k+1}
