function writeGroundtruthGraphFile2(filePath,unique3DPoints)
...
for i = 1:28
    ...
    for j = 1:length(row)
        ...
        elseif pointLabel >= 5
            ...
            if ~staticPointsWritten(pointLabel-4)
                pointsSeenSoFar = [pointsSeenSoFar, pointID];
                fid = fopen(strcat(filePath,'icra18GT_GraphFile.graph'),'a');
                fprintf(fid,'%s %d %6f %6f %6f','VERTEX_POINT_3D',...
                    pointVertexID(pointVertexID(:,1)==pointID,2),...
            ...
            staticPointsWritten(pointLabel-4) = 1;
        end
