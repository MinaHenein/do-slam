function constantSE3ObjectMotion = vKitti_objectMotion(objectPosesMatrix)
...
for i=1:nObjects
    ...
    for k=2:size(objectPoses,1)/4
        objectMotion = AbsoluteToRelativePoseR3xso3GlobalFrame(...
            transformationMatrixToPose(objectPosesWorldFrame(mapping(j-1,4),:)),...
            transformationMatrixToPose(objectPosesWorldFrame(mapping(j,4),:)));
