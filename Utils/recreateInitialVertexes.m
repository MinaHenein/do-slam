...
function initialCell = recreateInitialVertexes(config,measurementsCell,groundTruthCell)
...
for i=1:size(initialCell,1)
    ...
    if ((size(initialCell,2)>1) && ~isempty(initialCell{i,2}) && strcmp(initialCell{i,2}{1},config.pointVertexLabel))
...
if ~isempty(points2)
    plotPoints2 = plot3(points2(1,:),points2(2,:),points2(3,:),'g.');
end
