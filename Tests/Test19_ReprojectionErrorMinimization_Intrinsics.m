%--------------------------------------------------------------------------
% Author: Mina Henein - mina.henein@anu.edu.au - 20/04/2018
% Contributors:
%--------------------------------------------------------------------------
...
config.set('groundTruthFileName' ,'groundTruthTest19.graph');
config.set('measurementsFileName','measurementsTest19.graph');
...
for i=1:nSteps
    for j=1:size(objectPts,2)
        ...
        checkVertex.value = RelativeToAbsolutePositionR3xso3Image(checkVertices{i}.value,...
            groundTruthEdges{3*i+j-1}.value,config.intrinsics);
...
graphN.saveGraphFile(config,'resultsTest19.graph');
