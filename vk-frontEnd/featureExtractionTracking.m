...
%globalFeatures.location3DNoisyDepth = [];
...
for i = 1:numel(imageRange)
    ...
    if strcmp(settings.dataset,'kitti')
        ...
        %maskFrameName = strcat(repmat('0',1,6-numel(num2str(frameNumber))),num2str(frameNumber),'.txt');;
    ...
    if i == 1
        ...
        %maskIm = dlmread(strcat(maskI, maskFrameName));
    ...
    if isempty(frames(i).features.location)
        ...
        if ~isempty(frameFeatures.location)
            ...
            %frames(i).features.location3DNoisyDepth = [frames(i).features.location3DNoisyDepth, frameFeatures.location3DNoisyDepth];
...
%     frames(i) = frame;
    ...
    if i < numel(imageRange)
        ...
        if strcmp(settings.dataset,'kitti')
            ...
            %nextFrameMaskName = strcat(repmat('0',1,6-numel(num2str(nextFrameNumber))),num2str(nextFrameNumber),'.txt');
        elseif strcmp(settings.dataset,'vkitti')
        ...
        %nextMaskIm = dlmread(strcat(maskI, nextFrameMaskName));
