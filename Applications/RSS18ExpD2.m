...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpD2_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpD2_measurements.graph');
...
print('RSS18ExpD2_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpD2_Environment','-dpdf')
% implay(frames);

    %% 4.a output video
v = VideoWriter('Data/Videos/RSS18ExpD2_sensor_environment.mp4','MPEG-4');
open(v)
writeVideo(v,frames);
close(v)
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpD2_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpD2_groundTruthNoSE3.graph')
...
%% 6. Solve
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpD2_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpD2_results.graph');
...
print('RSS18ExpD2_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpD2_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpD2_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpD2_results.graph');
...
print('RSS18ExpD2_Solution','-dpdf')