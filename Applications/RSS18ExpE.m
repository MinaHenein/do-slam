...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpE_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpE_measurements.graph');
...
print('RSS18ExpE_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpE_Environment','-dpdf')
% implay(frames);

    %% 4.a output video
v = VideoWriter('Data/Videos/RSS18ExpE_sensor_environment.mp4','MPEG-4');
open(v)
writeVideo(v,frames);
close(v)
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpE_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpE_groundTruthNoSE3.graph')
...
% config.set('sortEdges',1);
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpE_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpE_results.graph');
...
print('RSS18ExpE_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpE_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpE_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpE_results.graph');
