...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpB2_SP_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpB2_SP_measurements.graph');
...
primitive2InitialPose_R3xso3 = [-15 -5 5 pi/2 0 0]';
...
nPoints = 50;
environment.addStaticPoints([40.*rand(1,nPoints)-30; 40.*rand(1,nPoints)-20; 20*ones(1,nPoints)]);
%% 3. Initialise Sensor
...
print('RSS18ExpB2_SP2_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpB2_SP2_Environment','-dpdf')
% implay(frames);
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpB2_SP_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpB2_SP_groundTruthNoSE3.graph')
...
%% 6. Solve
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpB2_SP_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpB2_SP_results.graph');
...
print('RSS18ExpB2_SP_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpB2_SP_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpB2_SP_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpB2_SP_results.graph');
