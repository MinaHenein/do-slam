...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpE_SP_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpE_SP_measurements.graph');
...
sin1 = [-15+0.125*t; 5 + 0.2*sin(t); 5 + 0.8*ones(1,nSteps)];
primitive1Waypoints = [linspace(0,1.3*tN,nSteps); sin1];

% primitive1InitialPose_R3xso3 = [-15 5 5 0 0 0]';
% primitive1Motion_R3xso3 = [0.1*dt; 0; 0; arot(eul2rot([0,0,0]))];

sin2 = [3-0.19*t; -5 + 0.2*sin(t); 2 + 0.8*ones(1,nSteps)];
primitive2Waypoints = [linspace(0,1.3*tN,nSteps); sin2];
...
nPoints = 50;
environment.addStaticPoints([25.*rand(1,nPoints)-20; 20.*rand(1,nPoints)-10; 20*ones(1,nPoints)]);
%% 3. Initialise Sensor
...
print('RSS18ExpE_SP_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpE_SP_Environment','-dpdf')
% implay(frames);

    %% 4.a output video
v = VideoWriter('Data/Videos/RSS18ExpE_SP_sensor_environment.mp4','MPEG-4');
open(v)
writeVideo(v,frames);
close(v)
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpE_SP_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpE_SP_groundTruthNoSE3.graph')
...
% config.set('sortEdges',1);
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpE_SP_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpE_SP_results.graph');
...
print('RSS18ExpE_SP_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpE_SP_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpE_SP_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpE_SP_results.graph');
...
print('RSS18ExpE_SP_Solution','-dpdf')