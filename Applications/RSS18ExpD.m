...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpD_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpD_measurements.graph');
...
square1 = [-15  -5 -15 -24 -15; ...
             5   5   5  5   5; ...
             5  15  24 15   5];
primitive1Waypoints = [linspace(0,tN,5); square1];

square2 = [ 3   3   3   3  3 ; ...
          -10 -25 -38 -25 -10; ...
            2 -13   2  15  2];
primitive2Waypoints = [linspace(0,tN,5); square2];

square3 = [-15   0 -15 -28 -15; ...
            -5  -5  -5  -5  -5; ...
             5 -10 -24 -10   5];
primitive3Waypoints = [linspace(0,tN,5); square3];

square4 = [-30 -30 -30 -30 -30; ...
           -10 -24 -38 -24 -10; ...
            2  -13   2  15  2];
primitive4Waypoints = [linspace(0,tN,5); square4];


% construct trajectories
robotTrajectory = PositionModelPoseTrajectory(robotTrajectoryWaypoints,'R3','smoothingspline');
primitive1Trajectory = PositionModelPoseTrajectory(primitive1Waypoints,'R3','linearinterp');
primitive2Trajectory = PositionModelPoseTrajectory(primitive2Waypoints,'R3','linearinterp');
primitive3Trajectory = PositionModelPoseTrajectory(primitive3Waypoints,'R3','linearinterp');
primitive4Trajectory = PositionModelPoseTrajectory(primitive4Waypoints,'R3','linearinterp');
...
print('RSS18ExpD_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpD_Environment','-dpdf')
% implay(frames);

    %% 4.a output video
v = VideoWriter('Data/Videos/RSS18ExpD_sensor_environment.mp4','MPEG-4');
open(v)
writeVideo(v,frames);
close(v)
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpD_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpD_groundTruthNoSE3.graph')
...
%% 6. Solve
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpD_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpD_results.graph');
...
print('RSS18ExpD_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpD_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpD_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpD_results.graph');
