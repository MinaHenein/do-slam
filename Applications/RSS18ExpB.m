...
%% 6. Solve
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpB_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpB_results.graph');
...
print('RSS18ExpB_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpB_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpB_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpB_results.graph');
