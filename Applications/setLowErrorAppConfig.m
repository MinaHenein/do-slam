function config = setLowErrorAppConfig(config)
...
% set pose prior
rot = eul2rot([pi/360,pi/360,pi/360]); % 0.5 degree position error
...
config.set('stdPointPrior',[0.005,0.005,0.005]');

% set odometry error
rot = eul2rot([pi/360,pi/360,pi/360]); % 0.5 degree position error
orientation = arot(rot);
config.set('stdPosePose'  ,[0.005,0.005,0.005,orientation']');

% measurement errors
config.set('stdPosePoint' ,[0.005,0.005,0.005]');
config.set('stdPointPlane',0.001);
% set properties of CameraConfig
% config.set('fieldOfView',[-pi/3,pi/3,-pi/6,pi/6,1,40]);
