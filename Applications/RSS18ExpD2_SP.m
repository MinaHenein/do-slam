...
config.set('t',t);
config.set('groundTruthFileName','RSS18ExpD2_SP_groundTruth.graph');
config.set('measurementsFileName','RSS18ExpD2_SP_measurements.graph');
...
nPoints = 50;
environment.addStaticPoints([50.*rand(1,nPoints)-30; 40.*rand(1,nPoints)-20; 20*ones(1,nPoints)]);
%% 3. Initialise Sensor
...
print('RSS18ExpD2_SP_PointVisibility','-dpdf')
%% 4. Plot Environment
figure
viewPoint = [-35,35];
...
print('RSS18ExpD2_SP_Environment','-dpdf')
% implay(frames);

    %% 4.a output video
v = VideoWriter('Data/Videos/RSS18ExpD2_SP_sensor_environment.mp4','MPEG-4');
open(v)
writeVideo(v,frames);
close(v)
...
config.set('constantSE3Motion',constantSE3ObjectMotion);
    ...
    config.set('measurementsFileName','RSS18ExpD2_SP_measurementsNoSE3.graph')
    config.set('groundTruthFileName','RSS18ExpD2_SP_groundTruthNoSE3.graph')
...
%% 6. Solve
    ...
    initialGraphN.saveGraphFile(config,'RSS18ExpD2_SP_resultsNoSE3.graph');
    ...
    %save results to graph file
    graphN.saveGraphFile(config,'RSS18ExpD2_SP_results.graph');
...
print('RSS18ExpD2_SP_H','-dpdf')

figure
spy(chol(solverEnd.systems(end).H))
print('RSS18ExpD2_SP_chol(H)','-dpdf')
...
%plot results
resultsNoSE3Cell = graphFileToCell(config,'RSS18ExpD2_SP_resultsNoSE3.graph');
resultsCell = graphFileToCell(config,'RSS18ExpD2_SP_results.graph');
...
print('RSS18ExpD2_SP_Solution','-dpdf')